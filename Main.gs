 ...
 *   - Setup.gs       : wizard "Setup Sistem Baru" (1 klik buat semua sheet)
 *                      + getInfoSistem (nama klien untuk ditampilkan di frontend)
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addItem('🚀 1. Setup Sistem Baru (Buat Semua Sheet)', 'pasangSistemBaru')
    .addItem('⚙️ 2. Pengaturan API Key & Folder Foto', 'aturPengaturan')
    .addItem('📥 3. Download Kode Web (untuk GitHub)', 'unduhKodeWeb')
    .addToUi();
}
...
function aturPengaturan() {
  ...
  var folderSudahOtomatis = !!props.getProperty('FOLDER_ID');
  var resFolder = ui.prompt(
    'Folder Foto (opsional)',
    folderSudahOtomatis
      ? 'Folder foto sudah dibuat otomatis oleh menu "Setup Sistem Baru". Kosongkan lalu OK untuk membiarkan tetap sama, atau isi ID folder lain untuk override manual:'
      : 'Masukkan ID Folder Google Drive untuk menyimpan foto (Ambil dari URL folder Drive).\nSaran: jalankan dulu menu "🚀 1. Setup Sistem Baru" supaya folder dibuat otomatis, kolom ini bisa dikosongkan:',
    ui.ButtonSet.OK_CANCEL
  );
  if (resFolder.getSelectedButton() !== ui.Button.OK) return;
  var teksFolder = resFolder.getResponseText().trim();
  if (teksFolder) props.setProperty('FOLDER_ID', teksFolder);
...
function doPost(e) {
  ...
  try {
    switch (action) {
      case 'infoSistem':
        return jsonResponse_(getInfoSistem());
      case 'daftarKaryawan':
      ...
      // ===== KELOLA DATA (khusus Admin Internal) =====
      case 'getDaftarKaryawanAdmin':
        return jsonResponse_(getDaftarKaryawanAdmin(body.password));
      case 'tambahKaryawanAdmin':
        return jsonResponse_(tambahKaryawanAdmin(body.password, body.nama, body.noHp, body.alamat, body.bagianKerja, body.pin));
      case 'editKaryawanAdmin':
        return jsonResponse_(editKaryawanAdmin(body.password, body.baris, body.nama, body.noHp, body.alamat, body.bagianKerja, body.pin));
      case 'hapusKaryawanAdmin':
        return jsonResponse_(hapusKaryawanAdmin(body.password, body.baris));

      case 'getDaftarLokasiAdmin':
        return jsonResponse_(getDaftarLokasiAdmin(body.password));
      case 'tambahLokasiAdmin':
        return jsonResponse_(tambahLokasiAdmin(body.password, body.nama, body.lat, body.lng, body.radius, body.satuan));
      case 'editLokasiAdmin':
        return jsonResponse_(editLokasiAdmin(body.password, body.baris, body.nama, body.lat, body.lng, body.radius, body.satuan));
      case 'hapusLokasiAdmin':
        return jsonResponse_(hapusLokasiAdmin(body.password, body.baris));

      case 'getDaftarTitikPatroliAdmin':
        return jsonResponse_(getDaftarTitikPatroliAdmin(body.password));
      case 'tambahTitikPatroliAdmin':
        return jsonResponse_(tambahTitikPatroliAdmin(body.password, body.namaPos, body.lat, body.lng, body.radius, body.satuan, body.kodeQr));
      case 'editTitikPatroliAdmin':
        return jsonResponse_(editTitikPatroliAdmin(body.password, body.baris, body.namaPos, body.lat, body.lng, body.radius, body.satuan, body.kodeQr));
      case 'hapusTitikPatroliAdmin':
        return jsonResponse_(hapusTitikPatroliAdmin(body.password, body.baris));

      case 'getDaftarTugasAdmin':
        return jsonResponse_(getDaftarTugasAdmin(body.password));
      case 'tambahTugasAdmin':
        return jsonResponse_(tambahTugasAdmin(body.password, body.namaTugas, body.area, body.lat, body.lng, body.radius, body.satuan, body.kodeQr, body.keterangan));
      case 'editTugasAdmin':
        return jsonResponse_(editTugasAdmin(body.password, body.baris, body.namaTugas, body.area, body.lat, body.lng, body.radius, body.satuan, body.kodeQr, body.keterangan));
      case 'hapusTugasAdmin':
        return jsonResponse_(hapusTugasAdmin(body.password, body.baris));

      case 'getDaftarJamKerjaAdmin':
        return jsonResponse_(getDaftarJamKerjaAdmin(body.password));
      case 'tambahJamKerjaAdmin':
        return jsonResponse_(tambahJamKerjaAdmin(body.password, body.bagianKerja, body.jamMasuk, body.toleransi));
      case 'editJamKerjaAdmin':
        return jsonResponse_(editJamKerjaAdmin(body.password, body.baris, body.bagianKerja, body.jamMasuk, body.toleransi));
      case 'hapusJamKerjaAdmin':
        return jsonResponse_(hapusJamKerjaAdmin(body.password, body.baris));
